    ...
    <div class="container" style="margin-top: 10px;">
        <div class="row">
            <div class="col-md-8 offset-2" style="background-color: rgba(229,236,225, 0.4); padding: 10px; border-radius: 5px;">
                <form action="{{route('restaurant_edit', $restaurant->id)}}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    {{csrf_field()}}
                    <div class="form-cut">
                        <label for="">ID</label>
                        <input type="text" name="id" id="" class="form-control" readonly value="{{$restaurant->id}}" >
                        <label for="">OD</label>
                        <input type="text" name="od" id="" class="form-control" value="{{$restaurant->od}}">
                        <label for="">Photo</label>
                        <br>
                        <input type="file" name="img" id="">
                        <br><br>
                        <input type="submit" name="" id="" value="Submit" class="btn btn-success">
                        <a href="{{route('restaurants')}}" class="btn btn-primary">Go Back</a>
                    </div>
                    <div class="form-cut">
                        <label for="">Menu Name</label>
                        <input type="text" name="name" id="" class="form-control" value="{{$restaurant->name}}">
                        ...
                        <input type="text" name="price" id="" class="form-control" value="{{$restaurant->price}}">

                    </div>
                    <div class="form-cut2">
                        <label for="">Description</label>
                        <br>
                        <textarea name="description" id="" cols="30" rows="10" style="width: 100%;">{!! $restaurant->description !!}</textarea>
                    </div>
                </form>
