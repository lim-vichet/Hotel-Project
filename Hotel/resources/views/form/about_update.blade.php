    ...
    <div class="container" style="margin-top: 10px;">
        <div class="row">
            <div class="col-md-8 offset-2" style="background-color: rgba(229,236,225, 0.4); padding: 10px; border-radius: 5px;">
                <form action="{{route('about_edit',$about->id)}}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    {{csrf_field()}}
                    <div class="form-cut">
                        <label for="">ID</label>
                        <input type="text" name="id" id="" class="form-control" readonly value="{{$about->id}}">
                        <br>
                        <input type="submit" name="" id="" value="Submit" class="btn btn-success">
                        <a href="{{route('abouts')}}" class="btn btn-primary">Go Back</a>
                    </div>
                    <div class="form-cut2">
                        <label for="">Description</label>
                        <br>
                        <textarea name="description" id="" cols="30" rows="10" style="width: 100%;">{!! $about->description !!}</textarea>
                    </div>
                </form>
